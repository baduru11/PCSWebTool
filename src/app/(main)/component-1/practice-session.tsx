...
export function PracticeSession({ questions, character }: PracticeSessionProps) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        audio.onended = () => {
          ...
          setDialogue("Now it's your turn! Try to pronounce it.");
        };
        audio.onerror = () => {
          URL.revokeObjectURL(audioUrl);
          setIsPlayingAudio(false);
      ...
      try {
        ...
        if (feedbackResponse.ok) {
          ...
          const fallback = isPerfect
            ? `Excellent! Your pronunciation of "${currentQuestion}" was perfect!`
            : isGood
            ? `Good job on "${currentQuestion}"! Keep practicing to improve further.`
            : `"${currentQuestion}" needs more practice. Listen again and try to match the tones.`;
  ...
  const handleNext = useCallback(() => {
    if (currentIndex + 1 >= questions.length) {
      setPhase("complete");
      setExpression("proud");
      setDialogue("Amazing work! You've completed all the characters. Let's see your results!");
    } else {
      ...
      setFeedbackText("");
      setExpression("neutral");
      setDialogue("Ready for the next character? Listen first, then try it yourself!");
    }
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-1">
        <div className="flex justify-between text-sm text-muted-foreground">
          <span>
            Character {currentIndex + 1} of {questions.length}
          </span>
        ...
        <div className="flex-1 lg:w-[70%]">
          <Card className="h-full">
            <CardContent className="flex flex-col items-center justify-center gap-6 py-8">
              ...
              <div className="text-center">
                <p className="text-8xl font-bold leading-none sm:text-9xl">
                  {currentQuestion}
                </p>
              </div>
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                {phase === "feedback" && (
                  <Button onClick={handleNext} size="lg">
                    {currentIndex + 1 >= questions.length ? "See Results" : "Next Character"}
                  </Button>
                )}
              </div>
            </CardContent>
